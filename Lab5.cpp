...
int main()
{
    ...
    a1->method_2(); //произойдет переопределение, вызываются методы объектов
    a2->method_2();
    a3->method_2();

    printf("\nМетод4 невиртуальный, метод3 - виртуальный(имена одинаковые)(метод4 вызывает метод3):\n");
    a1->method_4(); //переопределения не произойдет, вызывается метод класса переменной
    a2->method_4();
    a3->method_4();
    ...
    a1->method_1(); //вызовется метод, соответствующий классу переменной
    a2->method_1();
    a3->method_1();
    ...
    Base k1;
    k1 = func_11(a1); //возвращаем значение без указателя
    cout << endl << endl;
    Base* k2 = func_22(a2); //нет ссылки на возвращаемый объект
    cout << endl;
    ...
    printf("\n\nУмные указатели:\n\tunique_ptr\n"); //как только указатель покинет область видимости, произойдет освобождение памяти
    {
        unique_ptr<Desc> f1(new Desc);
    }
    printf("\n\tshared_ptr:\n"); //позволяет нескольким указателям обладать объектом одновременно
    {
